...
class Eloom_Bling_Model_Shipment extends Mage_Core_Model_Abstract {
  ...
  public function createShipment(Mage_Sales_Model_Order $order, $trackingNumber, $shippingMethodCode) {
    if (!$order->hasShipments()) {
      $this->logger->info(sprintf("Bling - Criando entrega %s para o pedido %s, com %s", $trackingNumber, $order->getId(), $shippingMethodCode));
      $itemQtys = array();
    ...
    if ($isTracking == 0 && $trackingNumber) {
      ...
      $this->logger->info(sprintf("Bling - Inserindo %s no pedido %s.", $trackingNumber, $order->getId()));

      try {
				switch ($shippingMethodCode) {
					case 'eloom_correios':
						if (Mage::helper('eloombootstrap/module')->isCorreiosSroExists()) {
