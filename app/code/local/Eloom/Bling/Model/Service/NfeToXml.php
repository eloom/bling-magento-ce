...
class Eloom_Bling_Model_Service_NfeToXml extends Mage_Core_Model_Abstract {
	...
	protected function _construct() {
		$this->config = Mage::getModel('eloom_bling/config');
		parent::_construct();
	}
	...
	public function parseXml($order) {
		 ...
		 * Parcelas
		 */
		$installments = Mage::getModel('eloom_bling/service_payment_proxy')->parseXml($this->order->getId(), $this->order->getPayment());
		if ($installments) {
	...
	protected function appendCliente($nfe) {
		...
		$tipoPessoa = Eloom_Bling_Enum_TipoPessoa::FISICA;
		if ($this->order->getCustomerTipoPessoa() != null && $this->order->getCustomerTipoPessoa() == Eloom_Bling_Enum_TipoPessoa::JURIDICA) {
			$tipoPessoa = Eloom_Bling_Enum_TipoPessoa::JURIDICA;
		}
		...
		if (!$helper->isEmpty($address->getCity())) {
			$cliente->appendChild($this->dom->createElement('cidade', $address->getCity()));
		}
		if (!$helper->isEmpty($address->getRegion())) {
